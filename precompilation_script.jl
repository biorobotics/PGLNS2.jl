...
function main()
  ...
  for i=1:2
    ARGS = [expanduser("~/GLKH-1.1/GTSPLIB/"*instance_folder*"/custom"*string(i)*".gtsp"), "-output=custom.tour", "-socket_port=65432", "-lazy_edge_eval=0", "-new_socket_each_instance=0", "-verbose=3", "-mode=fast"]
    ...
    @time GLNS.main(ARGS, 10., 298309430, given_initial_tours, false, "", dist)
    @time GLNS.main(ARGS, 10., 298309430, PyArray{Int64, 1, true, true, Int64}(given_initial_tours), false, "", PyArray{Int64, 2, true, true, Int64}(dist))
  end
end
