...
function main()
  ...
  for i=1:2
    ARGS = [expanduser("~/GLKH-1.1/GTSPLIB/"*instance_folder*"/custom"*string(i)*".gtsp"), "-output=custom.tour", "-socket_port=65432", "-lazy_edge_eval=0", "-new_socket_each_instance=0", "-verbose=3", "-mode=fast", "-num_iterations=60", "-latest_improvement=15", "-first_improvement=10", "-max_removal_fraction=0.1", "-max_removals_cap=20"]
    ...
    if i == 1
      ...
      @time GLNS.main(PyList{Any}(cat(ARGS, ["-budget="*string(inf_val - 1)], dims=1)), 10., inf_val, PyArray{Int64, 1, true, true, Int64}(Vector{Int64}()), false, "", PyArray{Int64, 2, true, true, Int64}(dist), false)
    end
