...
function main()
  ...
  max_ps_num = 1000
  ps_str = ""
  for ps_num=1:max_ps_num
    ps_str = "r"*string(ps_num, pad=3)*"ps"
    if isdir(ps_str) && !isdir("r"*string(ps_num + 1, pad=3)*"ps") && !isdir("r"*string(ps_num + 1, pad=3)*"ps-bad")
      break
    end
  end

  cmd = `vtune -r /home/cobra/GLNS_lazy_edge_eval.jl/$ps_str -command resume`
