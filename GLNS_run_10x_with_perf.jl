...
function main()
  ...
  num_runs = 10
  for i=1:num_runs
    perf_file = "perf"*string(i)*".txt"
    @printf("Starting run %d\n", i)
    GLNS.solver(problem_instance, TCPSocket(), given_initial_tours, time_ns(), 9999, evaluated_edges, true, num_vertices, num_sets, sets, dist, membership, instance_read_time, cost_mat_read_time, true, perf_file; optional_args...)
